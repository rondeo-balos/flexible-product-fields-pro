...
use Exception;
use FPFProVendor\Monolog\Formatter\LineFormatter;
use FPFProVendor\Monolog\Logger;
use FPFProVendor\Monolog\Utils;
use FPFProVendor\PhpConsole\Connector;
use FPFProVendor\PhpConsole\Handler;
use FPFProVendor\PhpConsole\Helper;
/**
 ...
 */
class PHPConsoleHandler extends \FPFProVendor\Monolog\Handler\AbstractProcessingHandler
{
     ...
     * @throws Exception
     */
    public function __construct(array $options = array(), \FPFProVendor\PhpConsole\Connector $connector = null, $level = \FPFProVendor\Monolog\Logger::DEBUG, $bubble = \true)
    {
        if (!\class_exists('FPFProVendor\\PhpConsole\\Connector')) {
    ...
    private function initConnector(\FPFProVendor\PhpConsole\Connector $connector = null)
    {
        if (!$connector) {
            if ($this->options['dataStorage']) {
                \FPFProVendor\PhpConsole\Connector::setPostponeStorage($this->options['dataStorage']);
            }
            $connector = \FPFProVendor\PhpConsole\Connector::getInstance();
        }
        if ($this->options['registerHelper'] && !\FPFProVendor\PhpConsole\Helper::isRegistered()) {
            \FPFProVendor\PhpConsole\Helper::register();
        }
        if ($this->options['enabled'] && $connector->isActiveClient()) {
            if ($this->options['useOwnErrorsHandler'] || $this->options['useOwnExceptionsHandler']) {
                $handler = \FPFProVendor\PhpConsole\Handler::getInstance();
    ...
    {
        if ($record['level'] < \FPFProVendor\Monolog\Logger::NOTICE) {
    ...
    private function handleDebugRecord(array $record)
    {
        ...
        if ($record['context']) {
            $message .= ' ' . \FPFProVendor\Monolog\Utils::jsonEncode($this->connector->getDumper()->dump(\array_filter($record['context'])), null, \true);
        }
    ...
    {
        return new \FPFProVendor\Monolog\Formatter\LineFormatter('%message%');
    }
}
