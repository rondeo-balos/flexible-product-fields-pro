...
use LogicException;
use FPFProVendor\Monolog\Handler\NullHandler;
use FPFProVendor\Monolog\Logger;
use FPFProVendor\Monolog\Registry;
use FPFProVendor\Monolog\Handler\StreamHandler;
use Psr\Log\LogLevel;
use FPFProVendor\WPDesk\Logger\WC\WooCommerceCapture;
use FPFProVendor\WPDesk\Logger\WP\WPCapture;
/**
 * Manages and facilitates creation of logger
 *
 * @package WPDesk\Logger
 */
class WPDeskLoggerFactory extends \FPFProVendor\WPDesk\Logger\BasicLoggerFactory
{
    ...
    public static function tearDown($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        if (\FPFProVendor\Monolog\Registry::hasLogger($name)) {
            \FPFProVendor\Monolog\Registry::removeLogger($name);
        }
    }
    /**
    ...
    public function disableLog($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        ...
        if (\FPFProVendor\Monolog\Registry::hasLogger($name)) {
            /** @var Logger $logger */
            $logger = \FPFProVendor\Monolog\Registry::getInstance($name);
    ...
    public function createWPDeskLogger($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        if (!self::$shouldLoggerBeActivated) {
            return new \FPFProVendor\Monolog\Logger($name);
        }
        if (\FPFProVendor\Monolog\Registry::hasLogger($name)) {
    ...
    private function captureWPLog()
    {
        static $wpCapture;
        if (!$wpCapture) {
            $wpCapture = new \FPFProVendor\WPDesk\Logger\WP\WPCapture(\basename($this->getFileName()));
     ...
     * @param Logger $logger
     */
    private function captureWooCommerce(\FPFProVendor\Monolog\Logger $logger)
    {
        if (!\defined('FPFProVendor\\WC_LOG_THRESHOLD')) {
            \define('FPFProVendor\\WC_LOG_THRESHOLD', self::LEVEL_WC);
        }
        $wcIntegration = new \FPFProVendor\WPDesk\Logger\WC\WooCommerceCapture($logger);
     ...
     */
    private function pushFileHandle($filename, \FPFProVendor\Monolog\Logger $logger)
    {
        $logger->pushHandler(new \FPFProVendor\Monolog\Handler\StreamHandler($filename, self::LEVEL_WPDESK_FILE));
    }
    /**
    ...
    public function getFileName($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        ...
        return \trailingslashit(\untrailingslashit($upload_dir['basedir'])) . \FPFProVendor\WPDesk\Logger\WP\WPCapture::LOG_DIR . \DIRECTORY_SEPARATOR . $name . '_debug.log';
    }
    /**
     ...
     * @return void
     */
    private function removeAllHandlers(\FPFProVendor\Monolog\Logger $logger)
    {
        try {
            while (\true) {
                $logger->popHandler();
            ...
            $logger->pushHandler(new \FPFProVendor\Monolog\Handler\NullHandler());
        }
    }
    /**
    ...
    {
        return \FPFProVendor\Monolog\Registry::hasLogger($name);
    }
}
