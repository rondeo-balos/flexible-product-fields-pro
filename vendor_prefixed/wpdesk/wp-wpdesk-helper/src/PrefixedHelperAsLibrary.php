...
namespace FPFProVendor\WPDesk\Helper;

use Psr\Log\LoggerInterface;
use FPFProVendor\WPDesk\Helper\Debug\LibraryDebug;
use FPFProVendor\WPDesk\Helper\Integration\LicenseIntegration;
use FPFProVendor\WPDesk\Helper\Integration\LogsIntegration;
use FPFProVendor\WPDesk\Helper\Integration\SettingsIntegration;
use FPFProVendor\WPDesk\Helper\Integration\TrackerIntegration;
use FPFProVendor\WPDesk\Helper\Logs\LibraryInfoProcessor;
use FPFProVendor\WPDesk\Helper\Page\LibraryDebugPage;
 ...
 */
class PrefixedHelperAsLibrary implements \FPFProVendor\WPDesk\PluginBuilder\Plugin\Hookable
{
    ...
    {
        $helper_info = new \FPFProVendor\WPDesk\Helper\HelperRemoveInfo();
    ...
    private function initialize()
    {
        ...
        $subscription_integration = new \FPFProVendor\WPDesk\Helper\Integration\LicenseIntegration();
        $subscription_integration->hooks();
        $settingsPage = new \FPFProVendor\WPDesk\Helper\Page\SettingsPage();
        $settings_integration = new \FPFProVendor\WPDesk\Helper\Integration\SettingsIntegration($settingsPage);
        $tracker_integration = new \FPFProVendor\WPDesk\Helper\Integration\TrackerIntegration($settingsPage);
        $logger_integration = new \FPFProVendor\WPDesk\Helper\Integration\LogsIntegration($settingsPage);
        ...
        (new \FPFProVendor\WPDesk\Helper\UpgradeSoonNotice())->show_info_about_upgrade_if_old_env();
        $this->clean_wpdesk_menu();
        $library_debug_info = new \FPFProVendor\WPDesk\Helper\Debug\LibraryDebug();
        (new \FPFProVendor\WPDesk\Helper\Page\LibraryDebugPage($library_debug_info))->hooks();
        self::$logger->pushProcessor(new \FPFProVendor\WPDesk\Helper\Logs\LibraryInfoProcessor($library_debug_info));
