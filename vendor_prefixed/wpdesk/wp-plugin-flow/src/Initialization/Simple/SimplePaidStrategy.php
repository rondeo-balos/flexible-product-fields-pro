...
namespace FPFProVendor\WPDesk\Plugin\Flow\Initialization\Simple;

use FPFProVendor\WPDesk\Helper\HelperRemover;
use FPFProVendor\WPDesk\Helper\PrefixedHelperAsLibrary;
use FPFProVendor\WPDesk\License\PluginRegistrator;
use FPFProVendor\WPDesk\Plugin\Flow\Initialization\ActivationTrait;
use FPFProVendor\WPDesk\Plugin\Flow\Initialization\BuilderTrait;
use FPFProVendor\WPDesk\Plugin\Flow\Initialization\PluginDisablerByFileTrait;
use FPFProVendor\WPDesk\Plugin\Flow\Initialization\InitializationStrategy;
use FPFProVendor\WPDesk\PluginBuilder\Plugin\ActivationAware;
use FPFProVendor\WPDesk\PluginBuilder\Plugin\SlimPlugin;
/**
 ...
 */
class SimplePaidStrategy implements \FPFProVendor\WPDesk\Plugin\Flow\Initialization\InitializationStrategy
{
     ...
     * @return SlimPlugin
     */
    public function run_before_init(\FPFProVendor\WPDesk_Plugin_Info $plugin_info)
    {
     ...
     * @return SlimPlugin
     */
    public function run_init(\FPFProVendor\WPDesk_Plugin_Info $plugin_info)
    {
        if (!$this->plugin) {
            $this->plugin = $this->build_plugin($plugin_info);
        }
        ...
        \add_action('plugins_loaded', function () use($registrator) {
            $this->init_helper();
            $is_plugin_subscription_active = $registrator instanceof \FPFProVendor\WPDesk\License\PluginRegistrator && $registrator->is_active();
            if ($this->plugin instanceof \FPFProVendor\WPDesk\PluginBuilder\Plugin\ActivationAware && $is_plugin_subscription_active) {
    ...
    private function register_plugin()
    {
        if (\apply_filters('wpdesk_can_register_plugin', \true, $this->plugin_info)) {
            $registrator = new \FPFProVendor\WPDesk\License\PluginRegistrator($this->plugin_info);
    ...
    private function prevent_older_helpers()
    {
        ...
        if (\apply_filters('wpdesk_can_remove_old_helper_hooks', \true, $this->plugin_info)) {
            (new \FPFProVendor\WPDesk\Helper\HelperRemover())->hooks();
        }
    }
    /**
    ...
    {
        (new \FPFProVendor\WPDesk\Plugin\Flow\Initialization\PluginDisablerByFileTrait('wpdesk-helper/wpdesk-helper.php'))->disable();
    }
}
